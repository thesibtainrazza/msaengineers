      ...
      <div className="container-primary">
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center ${service.reverse ? 'lg:flex-row-reverse' : ''}`}>
          ...
          <div className={`flex flex-col ${service.reverse ? 'lg:order-2' : 'lg:order-1'}`}>
            <h2 className="mb-6 text-[3.5rem] font-[450] leading-[1.1] tracking-[-0.04em] text-[#4b4b4b] md:text-[4.5rem]">
              {service.title}
            </h2>
            <p className="mb-6 text-[2rem] font-[450] leading-[1.3] text-[#4b4b4b]">
              {service.subtitle}
            </p>
            {/* Split description safely if there's a specific highlighted word or just render it */}
            <p className="mb-10 text-[1.6rem] font-light leading-[1.7] text-[#5f5f5f]">
               {service.description}
            </p>

            <div className="bg-[#f7f7f5] p-10 md:p-12 rounded-xl mb-10">
                ...
                <ul className="space-y-4">
                    {service.listItems.map((item, idx) => (
                        <li key={idx} className="flex items-start">
                            <span className="mr-4 mt-2 h-2 w-2 flex-shrink-0 rounded-full bg-[#ed1c24]"></span>
                            <span className="text-[1.6rem] font-light text-[#5f5f5f]">{item}</span>
                        </li>
                    ))}
                </ul>
            </div>
            ...
            <div>
                <a href="/contact" className="inline-flex items-center justify-center bg-white border border-[#ed1c24] rounded-full px-10 py-4 text-[#4b4b4b] hover:bg-[#ed1c24] hover:text-white transition-colors group">
          ...
          <div className={`relative w-full ${service.reverse ? 'lg:order-1' : 'lg:order-2'}`}>
            {/* Red decorative line */}
            <div className={`hidden lg:block absolute top-[10%] w-[40%] h-[2px] bg-[#ed1c24] z-20 ${service.reverse ? 'right-[-20%]' : 'left-[-20%]'}`}></div>
