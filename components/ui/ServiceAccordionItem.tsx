...
export const ServiceAccordionItem = ({ title, isActive = false }: { title: string, isActive?: boolean }) => {
  return (
    <div className={`w-full border-b border-gray-300 py-8 md:py-10 flex items-center justify-between cursor-pointer group transition-colors ${isActive ? 'text-[#ed1c24]' : 'text-[#4a4a4a] hover:text-[#ed1c24]'}`}>
       <span className="text-3xl md:text-4xl lg:text-[2.5rem] font-light tracking-tight">{title}</span>
       <div className={`w-6 h-6 md:w-8 md:h-8 rounded-full border flex items-center justify-center transition-colors shrink-0 ${isActive ? 'border-[#ed1c24]' : 'border-gray-400 group-hover:border-[#ed1c24]'}`}>
